    ...
    <div className="pt-24 pb-20 min-h-screen bg-stone-50 dark:bg-dark-950 transition-colors duration-300">
      <div className="container mx-auto px-6 md:px-40">
        ...
        <div className="grid lg:grid-cols-2 gap-12 mb-20">
          ...
          >
            ...
            <div className="grid grid-cols-4 gap-4">
              {[product.image, ...relatedProducts.map(p => p.image)].slice(0,4).map((img, idx) => (
          ...
          >
            <div className="mb-auto">
              <div className="flex justify-between items-start mb-4">
                <div>
                   <h1 className="font-serif text-4xl md:text-5xl text-stone-900 dark:text-stone-100 mb-2">{product.name}</h1>
                   <div className="flex items-center gap-4 text-sm">
                      <div className="flex text-gold-500">
                        {[1,2,3,4,5].map(i => <Star key={i} size={16} fill="currentColor" />)}
                      </div>
                      <span className="text-stone-500">(128 đánh giá)</span>
                      <span className="text-green-500 flex items-center gap-1"><ShieldCheck size={14}/> Chính hãng</span>
                   </div>
                </div>
              ...
              <div className="text-3xl font-serif text-gold-600 dark:text-gold-400 font-medium mb-6 flex items-end gap-2">
                 ...
                 <span className="text-sm text-stone-400 line-through font-sans mb-1 opacity-60">
                   {(parseInt(product.price.replace(/\D/g, '')) * 1.2).toLocaleString('vi-VN')}đ
                 </span>
              </div>

              <p className="text-stone-600 dark:text-stone-300 leading-relaxed mb-8 text-lg font-light">
                {product.description} <br/>
                Sự kết hợp hoàn hảo giữa nguyên liệu tuyển chọn và công nghệ chế biến hiện đại, mang đến trải nghiệm ẩm thực dưỡng nhan đẳng cấp ngay tại nhà.
              </p>

              {/* Purchase Options */}
              <div className="bg-white dark:bg-white/5 p-6 rounded-2xl border border-stone-200 dark:border-white/5 mb-8">
                <div className="flex items-center justify-between mb-6">
                  <span className="font-medium text-stone-700 dark:text-stone-200">Số lượng</span>
                ...
                <div className="mt-4 flex items-center justify-center gap-6 text-xs text-stone-500">
                   <span className="flex items-center gap-1"><Zap size={12}/> Giao hàng siêu tốc 2h</span>
                   <span className="flex items-center gap-1"><ShieldCheck size={12}/> Đổi trả trong 7 ngày</span>
                </div>
              </div>
            </div>
            ...
            <div className="border-t border-stone-200 dark:border-white/10 pt-6">
               <div className="flex gap-8 mb-6 border-b border-stone-200 dark:border-white/10">
                 {['description', 'ingredients', 'guide'].map((tab) => (
                   ...
                   >
                     {tab === 'description' ? 'Chi tiết' : tab === 'ingredients' ? 'Thành phần' : 'Hướng dẫn'}
                    ...
                    {activeTab === 'guide' && (
                      <motion.div key="guide" initial={{opacity: 0, y: 10}} animate={{opacity: 1, y: 0}} exit={{opacity: 0, y: -10}}>
                         <div className="flex gap-4 items-start bg-stone-100 dark:bg-white/5 p-4 rounded-xl">
                            ...
                            <div>
                               <p className="font-medium text-stone-900 dark:text-stone-100 mb-1">Công nghệ tự sôi 3 bước</p>
                               <p>{product.usage || "1. Xé gói kích nhiệt đặt vào đáy. 2. Đổ nước. 3. Đặt bát chè lên và chờ 8 phút."}</p>
                            </div>
                         </div>
                      </motion.div>
                    )}
        ...
        <div className="pt-12 border-t border-stone-200 dark:border-white/10">
           <h3 className="font-serif text-3xl text-stone-900 dark:text-stone-100 mb-8">Có thể bạn sẽ thích</h3>
